...
// Param void* "AthreadToAdd": pointer to thread function address (Returns void and takes into input void as well)
...
sched_ErrCode_t G8RTOS_Add_APeriodicEvent(void (*AthreadToAdd)(void), uint8_t priority, int32_t IRQn) {
    // Disable interrupts:
        uint32_t IBit_State_add_a_periodic_event;
        IBit_State_add_a_periodic_event = StartCriticalSection();

    // Check if IRQn is valid
        if (IRQn >= 155 || IRQn < 0) {
            //Must end critical section
            EndCriticalSection(IBit_State_add_a_periodic_event);
